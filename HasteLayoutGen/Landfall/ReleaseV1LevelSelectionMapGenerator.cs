...
{
    public static class ReleaseV1LevelSelectionMapGenerator
    {
        ...
        public static (List<LevelSelectionNode>, List<LevelSelectionPath>) Generate(GenerationInfo generationInfo, Random random)
        {
            ...
            void SetPercentageRandomToType(LevelSelectionNode.NodeType type, float percentage)
            {
                ...
                {
                    var candidates = (from n in levelNodes.Skip(1) where AllowedToConvertNode(n, type, levelPaths) select n).ToList();
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                {
                    var dist = path.From.Position - path.To.Position;
                    if (path.From.Depth > 0)
                    {
                        path.From.Position += (Vector3.UnitX * -1) * dist.X * 0.03f;
                    }
                    if (path.To.Depth > 0)
                    {
                        path.To.Position -= (Vector3.UnitX * -1) * dist.X * 0.03f;
                    }
                    var siblings = levelNodes.FindAll(x => x.Depth == path.From.Depth);
                    var from = path.From;
                    foreach (var sibling in siblings)
                    {
                        if (sibling != from)
                        {
                            var nodeSide = 0f - MathF.Sign(sibling.Position.X - from.Position.X);
                            var distance = MathF.Abs(sibling.Position.X - from.Position.X);
                            var maxDist = 10f;
                            var scale = UnityMathf.InverseLerp(maxDist, 0, distance);
                            from.Position += Vector3.UnitX * scale * nodeSide * maxDist * 0.25f;
                        }
                    }
                }
            }
        ...
        private static void MakeShops(GenerationInfo generationInfo, int depth, Random random)
        {
            ...
            var shopFrequency = dFirstLastShop / (budget + 1);
                    ...
                    if (nodesOnThisDepth.Any())
                    {
                        ...
                        foreach(var node in nodesOnThisDepth)
                        {
                            if ((node == ran || random.NextFloat() > .5f)
                                && AllowedToConvertNode(node, LevelSelectionNode.NodeType.Shop, generationInfo.Paths))
                            {
